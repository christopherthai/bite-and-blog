...
function MealFeedback({ feedback = [] }) {
  ...
  return (
    <div style = {{marginTop: '20px', padding:"20px"}}>
      <h3 style = {{marginTop: '0', borderBottom: '1px solid #ccc', paddingBottom: '10px'}}>User Feedback</h3>
      {feedback.length > 0 ? (
            ...
            {feedback.map((fb, index) => (
                <li key={index} style= {{borderBottom: '1px solid #eee', padding: '10px 0'}}>
                    <div style = {{ marginBottom:'5px'}}>
                        ...
                        <span style={{ marginLeft: '40px', color: '#757575', fontSize: '0.9em' }}>{fb.date}</span>
                    </div>
                    <div style={{ marginBottom: '5px' }}>
                        <StarRatings
                            rating={fb.rating}
